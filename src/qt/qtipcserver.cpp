...
static bool ipcScanCmd(int argc, char *argv[], bool fRelay)
{
    ...
    {
        if (boost::algorithm::istarts_with(argv[i], "gencoin:"))
        {
...
void ipcInit(int argc, char *argv[])
{
    ...
    try {
        ...
        // Make sure we don't lose any gencoin: URIs
        for (int i = 0; i < 2; i++)
        {
        ...
        // Make sure only one gencoin instance is listening
        message_queue::remove(FREICOINURI_QUEUE_NAME);
        delete mq;
